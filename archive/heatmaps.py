...
def generate_combined_heatmap(args):
    ...
    # Create a temporary merged matrix file
    merged_matrix = os.path.join(output_dir, f"{category_name}_merged_matrix.gz")
    
    # Merge matrices using computeMatrixOperations
    merge_cmd = f"""
    ...
    # Generate heatmap using merged matrix
    ...
    plotHeatmap \
        ...
        --sortRegions descend \
        --heatmapHeight 15 \
        --heatmapWidth {2 * len(matrix_files)} \
        --xAxisLabel "" \
        --refPointLabel "TSS" \
        --startLabel "" \
        --endLabel "TES" \
        --legendLocation none \
        --samplesLabel {' '.join([os.path.basename(f).split('_')[1] for f in matrix_files])} \
        --dpi 300 \
    ...
    subprocess.run(heatmap_cmd, shell=True, check=True)
    
    # Clean up temporary merged matrix
    os.remove(merged_matrix)
