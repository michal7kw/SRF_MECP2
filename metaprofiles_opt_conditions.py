...
def generate_metaprofile(args):
    """Generate single metaprofile comparing conditions"""
    tissue, category, bigwigs_endo, bigwigs_exo, bed_file, output_dir, threads = args
    ...
    if not os.path.exists(matrix_file):
        # Combine all bigwigs from both conditions but don't average
        all_bigwigs = bigwigs_endo + bigwigs_exo
        
        cmd_matrix = f"""
        computeMatrix scale-regions -S {' '.join(all_bigwigs)} \
            -R {bed_file} \
            -b 5000 -a 5000 \
    ...
    # Generate plot
    sample_labels = [f"Endo_{i+1}" for i in range(len(bigwigs_endo))] + \
                   [f"Exo_{i+1}" for i in range(len(bigwigs_exo))]
    
    cmd_plot = f"""
    plotProfile -m {matrix_file} \
        --perGroup \
        --colors {' '.join(['red'] * len(bigwigs_endo) + ['blue'] * len(bigwigs_exo))} \
        --plotTitle "{tissue} {category}-regulated genes (Endo vs Exo)" \
        --legendLocation "upper-right" \
        --samplesLabel {' '.join(f'"{label}"' for label in sample_labels)} \
        -out {profile_file}
    """
...
def main(args):
    ...
    bam_files = samples_endo['bamReads'].tolist() + samples_exo['bamReads'].tolist()
    if args.test_run:
    ...
    metaprofile_args = []
    for tissue in tissues:
        # Get bigwigs for each condition and tissue
        tissue_samples_endo = samples_endo[samples_endo['Tissue'] == tissue]
        tissue_samples_exo = samples_exo[samples_exo['Tissue'] == tissue]
        
        tissue_bigwigs_endo = [bw for bw, sample in zip(bigwig_files, bam_files)
                              if sample in tissue_samples_endo['bamReads'].tolist()]
        tissue_bigwigs_exo = [bw for bw, sample in zip(bigwig_files, bam_files)
                             if sample in tissue_samples_exo['bamReads'].tolist()]
        ...
        for category, bed_file in tissue_data[tissue]['bed_files'].items():
            ...
            if not os.path.exists(profile_file):
                metaprofile_args.append((
                    ...
                    bed_file, args.output_dir, threads_for_meta
                ))
            else:
